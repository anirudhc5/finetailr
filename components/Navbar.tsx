...
export default function Navbar() {
  ...
  const getLinkClass = (path: string) => {
    const baseClass = "font-body-md text-body-md transition-colors duration-200";
    const isActive = pathname === path;
    return `${baseClass} ${
      isActive ? "text-primary font-semibold border-b-2 border-primary pb-1" : "text-on-surface-variant hover:text-primary"
    }`;
  };
    ...
    <header className="bg-surface-container-lowest border-b border-outline-variant fixed top-0 left-0 right-0 z-50">
      <div className="flex justify-between items-center px-lg py-md max-w-container-max mx-auto w-full">
        <div className="flex items-center gap-xl">
          <Link href={logoHref} className="text-headline-md font-headline-md text-primary font-bold">
            FineTailr
          </Link>
          {user && (
            <nav className="hidden md:flex gap-lg items-center">
              ...
              <Link className={getLinkClass("/tailor")} href="/tailor">
                Tailor
              </Link>
